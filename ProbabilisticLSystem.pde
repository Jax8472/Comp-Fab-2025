 ...
 // Production rules
  ...
  public void iterate() {
    ...
    for (int i = 0; i < current.length(); i++) {
      ...
      //rules.get(c) 
     if (this.rules.get(c) != null) { //bacuse if its not defined and null is returned, its added to the string and extra unknwn what to do 
  ...
  public void drawLSystem(Turtle t) {
    ...
    for (int i = 0; i < currentIteration.length(); i++) {
      ...
      switch (c) {
        case 'F':
          t.forward(random(dist+10));
          break; // The "break" exits out of the switch statement and prevents the next cases from running
        case 'B':
          t.back(random(dist+10));
          break;
        case '+':
          t.right(random(0, rotateAngle+10));
          break;
        case '-':
          t.left(random(0, rotateAngle+10)); 
          break;
        case '[':
          t.push();
          break;
        ...
        case 'y':
          ...
          break;
        case '<':
          float distD = dist / scaleFactor;
          t.forward(distD);
          break;
        case '>':
          float distM = dist * scaleFactor;
          t.forward(distM);
          break;
        default:
